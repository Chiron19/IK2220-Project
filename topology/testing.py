...
def http_test(client, method, method2, payload="", expected=True):
    """
    http test, using curl function but with the virtual IP address and methods other than "GET"
    """
    cmd = f"curl --connect-timeout 3 --max-time 3 -X {method} -d '{payload}' -s 100.0.0.45{method2} > /dev/null 2>&1; echo $? "
    ret = client.cmd(cmd).strip()
